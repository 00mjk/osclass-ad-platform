    ...
    <form  name="item" action="<?php echo osc_base_url(true);?>" method="post" enctype="multipart/form-data">
      ...
      <fieldset class="general i-shadow round3" data-step-id="1">
        ...
        <?php if($category_type == 1 || $category_type == '') { ?>
          ...
          <div class="row category flat">
            <input id="catId" type="hidden" name="catId" value="<?php echo (stela_get_session('sCategory') <> '' ? stela_get_session('sCategory') : Params::getParam('sCategory')); ?>">
        ...
        <?php if( osc_price_enabled_at_items() ) { ?>
          <div class="price-wrap<?php if(function_exists('radius_map_publish')) { ?> has-map<?php } ?>">
            <div class="inside">
              <div class="enter">
                <label class="price-title"><?php _e('Price', 'stela'); ?></label>
        ...
        <div class="status-wrap<?php if(function_exists('radius_map_publish')) { ?> has-map<?php } ?>">
          <div class="transaction" style="display:block !important;">
            <label for="sTransaction"><?php _e('Transaction', 'stela'); ?></label>
            <?php echo stela_simple_transaction(); ?>
          </div>

          <div class="condition" style="display:block !important;">
        ...
        <?php 
          ...
          $prepare['i_country'] = stela_get_session('sCountry') <> '' ? stela_get_session('sCountry') : osc_user_field('fk_c_country_code');
          $prepare['i_region'] = stela_get_session('sRegion') <> '' ? stela_get_session('sRegion') : osc_user_region_id();
          $prepare['i_city'] = stela_get_session('sCity') <> '' ? stela_get_session('sCity') : osc_user_city_id();
          $prepare['s_phone'] = stela_get_session('sPhone') <> '' ? stela_get_session('sPhone') : osc_user_phone();
        ?>
          ...
          <div class="location<?php if(function_exists('radius_map_publish')) { ?> has-map<?php } ?>">
            ...
            <div class="row">
              ...
              <div id="location-picker">
                <input type="text" name="term" id="term" class="term" placeholder="<?php _e('Country, Region or City', 'stela'); ?>" value="<?php echo stela_get_term(stela_get_session('term'), $prepare['i_country'], $prepare['i_region'], $prepare['i_city']); ?>" autocomplete="off"/>
      ...
      <fieldset class="hook-block i-shadow round3" style="display:none;" data-step-id="3">
        <h2>
          ...
          <span class="text"><?php _e('Detalii', 'stela'); ?></span>
          <?php }else{?> <span class="text"><?php _e('Promote','stela'); ?></span> <?php } ?>
    ...
    <div style="clear:both"></div>
      ...
      <div class="buttons-block">
        <div class="inside">
          ...
          <button type="submit" class="btn btn-primary" ><?php _e('Publish item', 'stela'); ?></button>
        </div>
      </div>
    </form>
    </div>
...
element.addEventListener("click", () => {
    ...
    var stare_Hide = new Array();
    var catExtraStelaHide = new Array();
    <?php 
      $e_array = stela_Stare_hide();
    ...
    <?php 
      $e_array = stela_extra_fields_hide();
    ...
    $(document).ready(function(){
      ...
      $('body').on('click change', '.simple-price-type .option, .simple-price-type > select.text', function() {
        ...
        if(type == 1 || type == 2) {
          ...
          $('.add_item .price-wrap .enter input#price').attr('readonly', false).attr('placeholder', '<?php echo osc_esc_js(__('Pret', 'stela')); ?>');;
          $('.add_item .price-wrap .enter .simple-select');
    ...
    $('body').on('click change', 'input[name="catId"], select#catId', function() {
      ...
      var result = '';
      var cate = [131,237,248,132,133,246,142,245,149,143,146,147,148,230,150,134,135,236,137,136,138,144,139,141,145,140];
      var catewarn = 201;
      ...
      if(cat_id != '' && cat_id != 0) {
        ...
        if(stare_Hide[cat_id] == 1) {
          $(".add_item .condition").fadeOut(200);
          $('input[name="sCondition"]').val('');
          $('#sCondition option').prop('selected', function() {
            return this.defaultSelected;
          });

          // $('.simple-transaction span.text span').text($('.simple-transaction .list .option.bold').text());
          // $('.simple-condition span.text span').text($('.simple-condition .list .option.bold').text());
          // $(' .list .option, .simple-condition .list .option').removeClass('selected');
          // $(' .list .option.bold, .simple-condition .list .option.bold').addClass('selected');

        } else {
          $(".add_item .condition").fadeIn(200);
        }
